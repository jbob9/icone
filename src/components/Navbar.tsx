...
const Navbar: React.FC = () => {
  ...
  return (
    <div className={`fixed z-50 left-0 right-0 flex justify-center transition-all duration-300 ${scrolled ? 'bottom-8 md:top-8 md:bottom-auto' : 'top-8'}`}>
      <nav className="bg-brand-cream/90 backdrop-blur-md text-brand-dark px-2 py-2 rounded-full shadow-xl border border-white/20 flex items-center gap-1">
        ...
        {navItems.map((item) => (
         ...
         <span className="md:hidden px-4 font-bold uppercase text-sm">{active}</span>
      </nav>
    </div>
  );
};
